...
class PartialFC(Module):
    ...
    def forward_backward(self, label, features, optimizer):
        ...
        with torch.no_grad():
            ...
            one_hot = one_hot.fill_(0.1 / (self.num_classes - 1))
            one_hot.scatter_(1, total_label[index, None], 0.9)
            
            # calculate loss
            grad_temp = grad[index] * one_hot
            loss = torch.zeros(grad.size()[0], 1, device=grad.device)
            grad_v = torch.sum(grad_temp, dim = 1)
            loss[index] = grad_v.unsqueeze(dim = 1)
